...
function getfields($objectId, $slugs){

    $fieldSlugs =  explode(',', $slugs);
    $fields = [];
    $object = TheObject::where('id',$objectId)->first();
    foreach ($fieldSlugs as $key => $slug) {
        ...
        if(!empty($field_db)) {
            ...
            $value = $value ? $value->pivot->value : null;
            array_push($fields,[$field_db->name => $value]);
        }
        $relation_db = ObjectTypeRelation::query()->where(['slug' => $slug, 'enable' => true])->first();
        if(!empty($relation_db)) {
            $values = $object->relation_value()->where('object_relations.relation_object', $relation_db->id)->get();
            $response=[];
            if($values->first()){
              foreach ($values as $key => $value) {
                $response[] = [$relation_db->name => $value->name];
              }
            }
            array_push($fields,$response);
        }
    }
