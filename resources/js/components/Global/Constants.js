...
export default Object.freeze({
    ...
    formatLog: function (key, value) {
        ...
        if (key === 'show_in_menu' && typeof value === 'boolean') {
            let ic = "fa-solid icon_list fa-circle-";

            if (value)
               return `<div class="bool-td-collapse-table"><i class="${ic}check success_icon"></i></div>`
            else return `<div class="bool-td-collapse-table"><i class="${ic}xmark error_icon"></i></div>`

        }

        if( key === 'images'){
            let objects = JSON.parse(value);
            let response = objects.map((img)=>{
                return `<div class="img-log" ><img src="${img.url}" alt="${img.name}"></div>`;
            })
            return `<div class="div-for-img-log">${response.join(' ')}</div>`;
        }
