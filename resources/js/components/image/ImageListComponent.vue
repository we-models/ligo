        ...
        <div class="img-card-background">
            <div v-for="item in pagination.data" v-if="!progress_list">
                <div class="card img-card-container">
                    ...
                    <div :class="'img-card' + (item.selected ? ' image-active' : '')"
                        :style="`background-image:url('` + item.small + `?size=small')`" v-on:click="setSelection(item)">
                    </div>
        ...
        fillList: function (uri) {
            ...
            fetch(this.encodeURL(uri)).then(response => response.json()).then((data) => {
                data.data = data.data.map((image) => {
                    if(this.selected.length === 0) image.selected = false;
                    else image.selected = this.selected.some((item) => item.id === image.id);
                    return image;
                });
